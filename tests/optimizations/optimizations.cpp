...
// get all times and sum the start times together, then the end times together
// and take the average
void printAverage(const std::vector<long long> &times,
                  const std::string &operation) {
  if (times.empty()) {
    return;
  }

  long long sum = std::accumulate(times.begin(), times.end(),
                                  0LL); // value 0 type long long
...
int main() {
  ...
  for (int i = 0; i < NUM_ITERATIONS; i++) {
    ...
    std::cout << "Iteration " << (i + 1) << ": " << duration.count() << " µs"
              << std::endl;
  }
  ...
  std::cout << std::endl
            << "Matrix Multiplication (SIMD Blocking)" << std::endl;
  std::cout << "Matrix Size: " << MATRIX_SIZE << "x" << MATRIX_SIZE
            << std::endl;
  std::cout << "Iterations: " << NUM_ITERATIONS << std::endl;
  ...
  std::cout << std::endl
            << "Matrix Multiplication (SIMD Blocking + Multi-Threading)"
            << std::endl;
  std::cout << "Matrix Size: " << MATRIX_SIZE << "x" << MATRIX_SIZE
            << std::endl;
  std::cout << "Iterations: " << NUM_ITERATIONS << std::endl;
